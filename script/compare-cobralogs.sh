...
HIST_COBRA_SI_DEST=/tmp/cmp-cobra/history_cobra_si/
HIST_COBRA_DEST=/tmp/cmp-cobra/history_cobra/
HIST_DBCOP_DEST=/tmp/cmp-cobra/history_dbcop/
COBRA_DEST=/tmp/cmp-cobra/cobra/
COBRA_SI_DEST=/tmp/cmp-cobra/cobra_si/
COBRA_NOGPU_DEST=/tmp/cmp-cobra/cobra_nogpu/
SI_DEST=/tmp/cmp-cobra/si/
CSV_DEST=/tmp/cmp-cobra/csv
DBCOP_DEST=/tmp/cmp-cobra/dbcop/
...
# cat > /tmp/cmp-cobra/cobra.conf.nogpu <<EOF
...
# LOGGER_PATH=/tmp/cmp-cobra/cobra/logger.log
# LOGGER_LEVEL=INFO
# LOG_FD_LOG=/tmp/cmp-cobra/cobra/log/
...
# for p in "${DATASETS[@]}"; do
  # hist="$HIST_DEST/$p"
  # timeout 180 java "-Djava.library.path=$COBRA_DIR/include/:$COBRA_DIR/build/monosat" -jar "$COBRA_DIR/target/CobraVerifier-0.0.1-SNAPSHOT-jar-with-dependencies.jar" mono audit /tmp/cmp-cobra/cobra.conf.nogpu $hist &> "${hist/$HIST_DEST/$COBRA_NOGPU_DEST}" || true
# done
