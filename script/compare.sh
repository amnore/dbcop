  ...
  for j in "${TXNS_PER_SESSION[@]}"; do
    for k in "${OPS_PER_TXN[@]}"; do
      for l in "${VARIABLES[@]}"; do
        PARAMS+=("${i}_${j}_${k}_${l}")
        # mkdir -p "$GENERATE_DEST/${i}_${j}_${k}_${l}"
        # "$DBCOP_DIR/target/release/dbcop" generate -d "/tmp/generate/${i}_${j}_${k}_${l}" -h $HISTORIES -n $i -t $j -e $k -v $l
      done
    done
  done
done
...
for p in "${PARAMS[@]}"; do
  ...
  if [ "${#time[@]}" -ne 0 ]; then
    ...
    AVG_TIME_COBRA_SI_NOGPU[$p]="180"
  fi
done

mkdir -p $CSV_DEST
echo "param,cobra,cobra(si),cobra(nogpu),cobra(si-nogpu),si,oopsla" > "$CSV_DEST/avg_time.csv"
for p in "${PARAMS[@]}"; do
  echo "$p,${AVG_TIME_COBRA[$p]},${AVG_TIME_COBRA_SI[$p]},${AVG_TIME_COBRA_NOGPU[$p]},${AVG_TIME_COBRA_SI_NOGPU[$p]},${AVG_TIME_SI[$p]},${AVG_TIME_DBCOP[$p]}" >> "$CSV_DEST/avg_time.csv"
done
